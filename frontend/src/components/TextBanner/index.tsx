...
const TextHeaderBanner: React.FC<TextHeaderBanner> = ({ children, textSize = 16, bgColor = 'bg-fiorid', link, external, target }) => {
    ...
    return (
        <div role='banner' className={`${bgColor} font-[Roboto] py-4 w-full shadow-[0_0_5px_#000]`}>
            <div className={`max-w-[1440px] w-full mx-auto text-[${textSize}px] leading-5 text-center text-white px-3`}>
                {content}
            </div>
        </div>
    )
